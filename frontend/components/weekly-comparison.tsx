...
export default function WeeklyComparison() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/stats/weekly-comparison`).then((r) => r.ok ? r.json() : null),
      fetch(`${API}/stats/daily-breakdown`).then((r)    => r.ok ? r.json() : null),
    ]).then(([comp, brk]) => {
      // Guards : s'assurer que les tableaux existent
      if (comp && !Array.isArray(comp.pillars))       comp.pillars  = []
      if (brk  && !Array.isArray(brk.current))        brk.current   = []
      if (brk  && !Array.isArray(brk.previous))       brk.previous  = []
      setComparison(comp)
      setBreakdown(brk)
      setLoading(false)
    }).catch(() => {
  ...
  if (error || !comparison || !breakdown) {
    return (
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        <p className="text-sm text-gray-600 text-center py-8">
          Comparaison semaine non disponible — pas encore assez de données.
        </p>
      </div>
    )
  }
  ...
  const currDays    = breakdown.current   ?? []
  const prevDays    = breakdown.previous  ?? []

  const pillarChartData = pillars.map((p) => ({
  ...
  const dayChartData = currDays.map((d, i) => ({
    day:              d.day,
    "Cette semaine":  d.score ?? 0,
    "Semaine passée": prevDays[i]?.score ?? 0,
  }))
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          {[
            ...
            },
            {
              ...
              trend:    (comparison.current_week.xp >= comparison.previous_week.xp ? "up" : "down") as "up" | "down",
            },
            {
              ...
              trend:    (comparison.current_week.checks >= comparison.previous_week.checks ? "up" : "down") as "up" | "down",
            },
          ].map((kpi) => (
            <div key={kpi.label} className="bg-gray-800 rounded-xl p-4">
              ...
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-0.5">vs {kpi.previous} semaine passée</p>
                </div>
                <div className="text-lg font-bold" style={{ color: trendColor(kpi.trend) }}>
                  {trendIcon(kpi.trend)}
                  <span className="text-sm ml-0.5">{kpi.delta > 0 ? "+" : ""}{kpi.delta}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="flex justify-between text-xs text-gray-600">
          <span>Semaine passée : {formatDate(comparison.previous_week.start)} → {formatDate(comparison.previous_week.end)}</span>
          <span>Cette semaine : {formatDate(comparison.current_week.start)} → {formatDate(comparison.current_week.end)}</span>
        </div>
      </div>
      ...
      {dayChartData.length > 0 && (
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
          <h3 className="text-sm font-semibold text-gray-300 mb-4">📈 Score jour par jour</h3>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={dayChartData}>
              ...
              <Line type="monotone" dataKey="Cette semaine"  stroke="#3b82f6" strokeWidth={2}   dot={{ fill: "#3b82f6", r: 4 }} activeDot={{ r: 6 }} />
              <Line type="monotone" dataKey="Semaine passée" stroke="#6b7280" strokeWidth={1.5} dot={{ fill: "#6b7280", r: 3 }} strokeDasharray="4 2" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {/* BarChart — piliers */}
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        <h3 className="text-sm font-semibold text-gray-300 mb-4">⚖️ Piliers — Cette semaine vs Semaine passée</h3>
            ...
            <div className="mt-4 space-y-2">
              {pillars.map((p) => (
                <div key={p.pillar_id} className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: p.pillar_color }} />
                  <span className="text-xs text-gray-400 flex-1">{p.pillar_name}</span>
                  <span className="text-xs text-gray-500">{p.current}%</span>
                  <span className="text-xs font-semibold w-12 text-right" style={{ color: trendColor(p.trend) }}>
